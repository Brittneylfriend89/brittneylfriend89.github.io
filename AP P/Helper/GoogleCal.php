        ...
        try{
           ...
           $data = json_decode(curl_exec($ch), true); 
            ...
            if ($http_code != 200) { 
                ...
                self::GetCalendarsList($calendarId, $access_token,$refresh_token,$employeeId,$eventdate);
                // if (curl_errno($ch)) { 
                //     $error_msg = curl_error($ch); 
                // } 
                // throw new Exception('Error '.$http_code.': '.$error_msg); 
        ...
        try {   
            ...
            if ($http_code != 200) { 
                ...
                if (curl_errno($ch)) { 
                    ...
                    $data = self::RefreshToken($client_id, $client_secret, $refreshToken, $employeeId);
                    $refresh_token = $data['access_token'];
                    self::getCalendarColors($access_token,$refresh_token,$employeeId);
                }
                Log::channel('daily')->info(json_encode($data, true));
                return array(
                    'status' => false,
   ...
   public static function getCalendarEvents($calendarId, $access_token, $refreshToken,$employeeId)
   {
        try {
            ...
            if ($http_code != 200) { 
                ...
                $data = self::RefreshToken($client_id, $client_secret, $refreshToken,$employeeId);
                $refresh_token = $data['access_token'];
                self::getCalendarEvents($calendarId,$access_token,$refresh_token,$employeeId);
                // if (curl_errno($ch)) { 
                //     $error_msg = curl_error($ch); 
                // }
