...
class PaymentController extends Controller
{
    ...
    public function pay($id,Request $request)
    {
        ...
        if(Auth::user()->role_id == 1)
            $payment = Payment::join('appointments','appointments.id','=','payments.appointment_id')->where('payments.id',$id)->first();
        else
            $payment = Payment::join('appointments','appointments.id','=','payments.appointment_id')->where('payments.id',$id)->where('appointments.employee_id',Auth::user()->id)->first();
        if(!$payment) {
            return redirect()->route('unauthorized');
        }
    ...
    public function employeepayment(Appointment $appointment)
    {
        ...
        $id = 1;
        $custom = Setting::first();
        $appointments = Appointment::select('appointments.*','p.status as pstatus','p.amount as amount')->join('payments as p','p.appointment_id','=','appointments.id')->where('appointments.employee_id',Auth::user()->id)->get();
        $payment_detail = payment::where('id',$id)->first();
        return view('payments.employee-payment',compact('appointments','custom','payment_detail'));
    }
